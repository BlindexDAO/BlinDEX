...
export function load() {
  task("disperse:ether").setAction(async (args, hre) => {
    // Enter wallet addresses and amounts to disperse
    ...
    const user = await getDeployer(hre);
    ...
    await printAndWaitOnTransaction(await disperse.disperseEther(wallets, amounts, { value }));
  ...
  task("disperse:bdx").setAction(async (args, hre) => {
    ...
    for (let index = 0; index < wallets.length; index++) {
      const walletAddress = wallets[index];
      console.log(`Wallet ${walletAddress} BDX balance is: ${(await bdx.balanceOf(walletAddress)).toString()}`);
    }
