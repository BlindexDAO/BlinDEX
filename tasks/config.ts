...
export function load() {
  task("show:be-config").setAction(async (args, hre) => {
    ...
    const stringifiedStakingRewards =
      "[" +
      stakingRewards
        .map(
          (reward) => `{ "pairAddress": "${reward.stakingTokenAddress.toLowerCase()}", "stakingRewardAddress": "${reward.address.toLowerCase()}" }`
        )
        .join(",") +
      "]";
    const {
    ...
    const stringifiedSwaps =
      "[" +
      pairs
        .map(
          (pair) =>
            `{ "pairAddress": "${pair.address.toLowerCase()}", "token0Address": "${pair.token0.toLowerCase()}", "token1Address": "${pair.token1.toLowerCase()}" }`
        )
        .join(",") +
      "]";
    const stringifiedPairOracles =
      "[" +
      pairOracles
        .map(
          (pairOracle) => `{ "pairAddress": "${pairOracle.pairAddress.toLowerCase()}", "oracleAddress": "${pairOracle.oracleAddress.toLowerCase()}"}`
        )
        .join(",") +
      "]";
