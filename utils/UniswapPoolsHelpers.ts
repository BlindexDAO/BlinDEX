...
import { getBdx, getWeth, getWbtc, getUniswapPairOracle, getBot, getAllBDStables, getBdUs, formatAddress } from "./DeployedContractsHelpers";
...
export async function getPools(hre: HardhatRuntimeEnvironment): Promise<{ name: string; token: IERC20 }[][]> {
  ...
  const externalUsdStableContract = (await hre.ethers.getContractAt("IERC20", formatAddress(hre, externalUsdStable.address))) as IERC20;
